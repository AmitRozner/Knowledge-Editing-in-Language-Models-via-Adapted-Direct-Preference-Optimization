...
import sys
sys.path.append([".."])
from accum_results_counterfact import accum_states
from accum_results_convsent import accum_states  as accum_states_all
import copy
...
class BaseEditor:
    ...
    def __init__(self,
        ...
        if type(self.model_name) is str:
            ...
            if 't5' in self.model_name.lower():
                ...
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name,trust_remote_code=True, device_map=device_map)
    ...
    def edit(self,
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE':
                ...
                elif (self.alg_name == 'LoRA' or (self.alg_name == "DPO" and isinstance(edited_model,PeftModelForCausalLM))) and keep_original_weight:
                    edited_model.unload()
                ...
                if i % 30 ==0:
                    result_dict_intermediate[f"iter{i}"]=all_metrics
                    if hasattr(args, 'datatype'):
                        if args.datatype in ["zsre","counterfact","wikibio","recent","trivia"]:
                            accum_states(all_metrics,os.path.join(args.metrics_save_dir, f'temp/{args.editing_method}_{args.datatype}_{args.add2path}_{args.hparams_dir.split("/")[-1].replace(".yaml","")}_results_intermediate.json'))
                    else:
                        if "convsent" in args.data_dir.lower():
                            accum_states_all(all_metrics,os.path.join(args.metrics_save_dir, f'temp/Convsent_{args.editing_method}_{args.hparams_dir.split("/")[-1].replace(".yaml","")}_results_intermediate.json'))
                        else:
                            accum_states_all(all_metrics,os.path.join(args.metrics_save_dir, f'temp/{args.editing_method}_{args.hparams_dir.split("/")[-1].replace(".yaml","")}_results_intermediate.json'))
    ...
    def seq_edit(self,
        ...
        for i, request in enumerate(requests):
            ...
            if isinstance(edited_model,PeftModelForCausalLM):
                self.model = edited_model.merge_and_unload()
                del self.model.peft_config
            else:
                self.model = edited_model
            exec_time = time() - start
        ...
        for i, request in enumerate(requests):
            ...
            all_metrics[i].update({
                ...
                "requested_rewrite": request,
                "time": exec_time,
                "post": compute_edit_quality(self.model, self.model_name, self.hparams, self.tok, request,
                                             self.hparams.device, test_generation=test_generation),
            })
            if "metric_kwargs" in kwargs:
                all_metrics[i].update(
        ...
        return all_metrics, edited_model, weights_copy,self.tok
        ...
        for i, request in tqdm(enumerate(requests)):
            ...
            if self.alg_name == 'IKE':
                ...
                if self.alg_name == 'KN' or self.alg_name == 'GRACE':
                    with torch.no_grad():
                        weights_copy() # unpatch_fn
                elif (self.alg_name == 'LoRA' or self.alg_name == "DPO") and keep_original_weight:
                    edited_model.unload()
                    del self.model.peft_config
                else:
                    with torch.no_grad():
