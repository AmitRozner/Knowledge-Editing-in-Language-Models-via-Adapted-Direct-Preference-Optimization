...
import json
import random
from easyeditor.models.dpo.dpo_hparams import DPOHyperParams
...
class Experimenter:
    ...
    def _LoRA_and_FT(self):
        ...
        if args.editing_method=="DPO" :
            _, edited_model, _ = self.editor.seq_edit(
    ...
    def _DPO(self):
        print_log(f"start edit using DPO with the answer `{self.args.specify_answer}` ...")
        ...
        for idx in range(len(train_data["prompt"])):
            prompt, target_new = train_data["prompt"][idx], train_data["target_new"][idx]#, train_data["subject"][idx]
            print_log(f"target_new: `{target_new}`    prompt: `{prompt}`")
        for idx in range(len(train_data["prompt"])):
            prompt, target_new = train_data["prompt"][idx], train_data["target_new"][idx]#, train_data["subject"][idx]
            ...
            _, edited_model, _ = self.editor.edit(
                ...
                ground_truth = train_data["ground_truth"][idx],
                args=args
            )
    ...
    def _prepare_dataset_before_edit(self, key:str, template:str=None) -> Dict:
        ...
        if key == 'train':
            ...
            elif self.args.editing_method in ['FT', 'LoRA', "DPO"]:
    ...
    def evaluate(self)->dict:
        ...
        print(metric)
        json.dump(metric, open(os.path.join(args.metrics_save_dir,f'{args.editing_method}_{args.datatype}_{self.hparams.model_name.split("/")[-1]}_{args.add2path}_identicalblocks_results.json'),'w'), indent=4)
...
def args_parser():
    ...
    parser.add_argument('--datatype', default='trivia', type=str)
...
if __name__ == "__main__":
    args = args_parser()
    check_args(args)
    editing_hparams = get_hparams(args)
    hparams = editing_hparams.from_hparams(args.hparams_dir)
    exp = Experimenter(args=args, template="Question:{}\nAnswer:",hparams=hparams)
    exp.run()
