...
if __name__ == "__main__":
    ...
    if args.ds_size is not None:
        args.pre_file = f"./{hparams.model_name.split('/')[-1]}_{args.datatype}_{args.ds_size}pre_edit_500.json"
    else:
        args.pre_file = f"./{hparams.model_name.split('/')[-1]}_{args.datatype}_pre_edit_500.json"
    print(args.pre_file)
    ...
    if args.seq:
        metrics, edited_model, _,tok = editor.seq_edit(
            prompts=prompts,
    ...
    if args.save_model:
        dir = "/scratch/dpo/checkpoints/"
        try:
             save_model_path = f'{dir}{args.editing_method}_{args.datatype}_{hparams.model_name.split("/")[-1]}_{args.add2path}_stopcrit{hparams.stop_crit}_lr{hparams.lr}_cyc{hparams.cycles}_numsteps{hparams.num_steps}_layer{str(hparams.layers[0])}_results_500/'
        except:
             save_model_path = f'{dir}{args.editing_method}_{args.datatype}_{hparams.model_name.split("/")[-1]}_{args.add2path}_results_500/'
        if not os.path.exists(save_model_path):
            os.mkdir(save_model_path)
        torch.save(edited_model.state_dict(), os.path.join(save_model_path , "pytorch_model.bin"))
        # Save the configuration and tokenizer
        edited_model.save_pretrained(save_model_path + "/model.safetensors")
        edited_model.config.save_pretrained(save_model_path + "/")
        tok.save_pretrained(save_model_path+ "/")

    if args.seq:
        try:
            json.dump(metrics, open(os.path.join(args.metrics_save_dir, f'SEQ_{args.editing_method}_{args.datatype}_{hparams.model_name.split("/")[-1]}_{args.add2path}_stopcrit{hparams.stop_crit}_lr{hparams.lr}_mask_{hparams.mask_where_equal}_cyc{hparams.cycles}_numsteps{hparams.num_steps}_nllterm_{hparams.nll_term}_layer{str(hparams.layers[0])}_regdpo{hparams.reg_dpo}_results_500.json'), 'w'), indent=4)
        except:
            json.dump(metrics, open(os.path.join(args.metrics_save_dir, f'SEQ_{args.editing_method}_{args.datatype}_{hparams.model_name.split("/")[-1]}_{args.add2path}_results_500.json'), 'w'), indent=4)
    else:
