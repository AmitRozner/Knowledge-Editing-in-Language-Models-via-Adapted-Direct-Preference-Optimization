...
if __name__ == "__main__":
    ...
    if args.ds_size is not None:
        args.pre_file = f"./{hparams.model_name.split('/')[-1]}_{args.datatype}_{args.ds_size}pre_edit.json"
    else:
    ...
    if args.seq:
        json.dump(metrics, open(os.path.join(args.metrics_save_dir, f'SEQ_{args.editing_method}_{args.datatype}_{hparams.model_name.split("/")[-1]}_{args.add2path}_stopcrit{hparams.stop_crit}_results.json'), 'w'), indent=4)
    else:
